...
class ANO_VQC_Model(nn.Module):
    ...
    def __init__(self, img_size):
        ...
        self.N = 2 ** args.k_local
        self.A = nn.ParameterList([nn.Parameter(torch.empty((self.N * (self.N - 1)) // 2)) for _ in range(n_qubits)])  # off-diagonal entries (real part)
        self.B = nn.ParameterList([nn.Parameter(torch.empty((self.N * (self.N - 1)) // 2)) for _ in range(n_qubits)])  # off-diagonal entries (imag part)
        self.D = nn.ParameterList([nn.Parameter(torch.empty(self.N)) for _ in range(n_qubits)])                        # diagonal elements (all real)
    ...
    def forward(self, X):
        ...
        self.H = [create_Hermitian(self.N, self.A[q], self.B[q], self.D[q]) for q in range(n_qubits)]
...
model = ANO_VQC_Model(img_size=args.resize_img).to(DEVICE)
...
experiment_name = f"ANO_sliding_{args.k_local}local_{n_qubits}qubits_depth{args.vqc_depth}_lr{args.lr:.0e}_lrH{args.lr_H:.0e}_{args.epochs}epochs_{timestamp}"
