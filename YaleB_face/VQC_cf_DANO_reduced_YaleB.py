...
def quantum_net(X, θ, n_class):
    ...
    # Expectation values in fixed basis: here Z for example
...
class pure_VQC(nn.Module):
    '''Pure VQC with fixed PauliZ measurements'''

    def __init__(self):
        super(pure_VQC, self).__init__()
        self.n_qubits = n_qubits
        self.n_class = num_classes
        self.θ = nn.Parameter(0.01 * torch.randn(args.vqc_depth, self.n_qubits))  # VQC rotation params

        self.dev = qml.device("lightning.qubit", wires=self.n_qubits)  # Can use different simulation backend or quantum computers.
    ...
    def forward(self, X):
        ...
        # VQC
        q_out = torch.stack([torch.stack(self.VQC(z, self.θ, self.n_class)).float() for z in z1])
        return q_out
...
# Create experiment folder with descriptive name BEFORE training
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
experiment_name = f"pure_VQC_YaleFace_PCA{n_qubits}d_depth{args.vqc_depth}_lr{args.lr:.0e}_{args.epochs}epochs_{timestamp}"
...
for epoch in range(args.epochs):
    ...
    epoch_checkpoint = {
        ...
        'metrics': {
            ...
            'valid_loss': metrics['valid_loss'][-1],
            'valid_acc': metrics['valid_accuracy'][-1],
            'test_loss': metrics['test_loss'][-1],
            'test_acc': metrics['test_accuracy'][-1]
        }
    }
    checkpoint_filename = f'epoch_{epoch + 1:03d}.pth'
    torch.save(epoch_checkpoint, os.path.join(checkpoints_path, checkpoint_filename))
...
with open(os.path.join(experiment_path, 'pure_VQC_results.pkl'), 'wb') as f:
    pickle.dump(experiment_data, f)

# Save final model checkpoint (same as last epoch, but at top level for convenience)
torch.save({
    'epoch': args.epochs,
    'model_state_dict': model.state_dict(),
    'optimizer_state_dict': optimizer.state_dict(),
...
print("\nGenerated files:")
print(f"  - {experiment_path}/pure_VQC_results.pkl")
print(f"  - {experiment_path}/model_checkpoint.pth (final epoch)")
print(f"  - {experiment_path}/checkpoints/epoch_XXX.pth ({args.epochs} epoch checkpoints)")